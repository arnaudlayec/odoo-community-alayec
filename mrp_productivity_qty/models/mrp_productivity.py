...
class MrpWorkCenterProductivity(models.Model):
    ...
    wo_duration_expected = fields.Float(related='workorder_id.duration_expected')
    wo_duration = fields.Float(string='Total Duration done', compute='_compute_performance')
    ...
    def _compute_performance(self):
        ...
        for record in self:
            ...
            record.wo_duration = wo.duration - record._origin.duration + record.duration
            ...
            record.wo_unit_time_real, record.wo_performance, record.wo_gain = _compute_metrics(
                record.wo_unit_time_avg, record.wo_duration, record.wo_qty_produced
            )
            record.wo_progress_qty = record.wo_qty_remaining and record.wo_qty_produced / record.wo_qty_remaining * 100 # as per qties
            record.wo_progress = record.wo_duration_expected and record.wo_duration / record.wo_duration_expected * 100 # as per times