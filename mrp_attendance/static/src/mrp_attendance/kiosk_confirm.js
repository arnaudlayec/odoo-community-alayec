odoo.define('mrp_attendance.kiosk_confirm', function (require) {
    "use strict";
    ...
    /* Why this ulgy *not* inheritance?
        Because in init(), native call to `window.addEventListener("keydown")` and ev.preventDefault() & ev.stopPropagation()
        cannot be overwritten neither canceled. It fully prevents using a few keys (1-0, Enter, Backspace and Delete)
        after the PIN screen, whether is activated or not in global settings.
    */

var KioskConfirmMrp = AbstractAction.extend({
    events: {
    ...
    init: function (parent, action) {
        ...
        window.addEventListener("keydown", (ev) => {
            ...
            const classList = ev.target.classList // CUSTOM CODE
            const key = ev.key;

            if (!allowedKeys.includes(key)) {
                return;
            } // == CUSTOM CODE ==
            else if (classList.contains('o_searchview_input') || classList.contains('o_input')) {
                return;
            }
...
core.action_registry.add('mrp_attendance_kiosk_confirm', KioskConfirmMrp);

return KioskConfirmMrp;
